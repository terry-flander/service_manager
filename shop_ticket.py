...
def generate_shop_ticket(job, job_parts, settings=None):
    """Generate an A4 landscape dual shop ticket PDF. Returns BytesIO.
    ...
    biz_name  = settings.get('business_name', 'ServiceDesk')
    biz_addr  = settings.get('business_address', '')
    biz_sub   = ' '.join(filter(None, [
        settings.get('business_suburb', ''),
        settings.get('business_state', ''),
        settings.get('business_postcode', ''),
    ]))
    biz_phone = settings.get('business_phone', '')

    header_lines = [biz_name]
    if biz_addr: header_lines.append(biz_addr)
    if biz_sub:  header_lines.append(biz_sub)
